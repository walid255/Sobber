...
export async function onRequestGet(context) {
  try {
    ...
    let raw = await safeKvGet(kv, 'sobber_timetable');
    let schedule = [];
      ...
      if (stateRaw) {
        try {
          ...
          if (Array.isArray(parsed.timetable)) {
            ...
            safeKvPut(kv, 'sobber_timetable', schedule).catch(() => {});
          }
        } catch {}
      }
    }
...
export async function onRequestPost(context) {
  try {
    ...
    let schedule = [];
    if (kv) {
      const raw = await safeKvGet(kv, 'sobber_timetable');
      if (raw) {
        try { schedule = JSON.parse(raw); } catch {}
      }
    }
    ...
    if (kv) {
      await safeKvPut(kv, 'sobber_timetable', schedule);
    ...
    return new Response(JSON.stringify({
      success: true,
      message: kv ? 'House timetable updated in SOBBER_KV' : 'Saved (local mode)',
    ...
    console.error('Timetable POST error:', err.message);
